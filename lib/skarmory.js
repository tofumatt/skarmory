...
Skarmory = function(instance_options) {
	...
	*/
	...
	this.getRequestToken = function(url) {
		...
		var request = {
			action: url,
			method: this.HTTP_POST, // OAuth requests are always POST
			parameters: []
		};
		...
		httpClient.open(this.HTTP_POST, url, false);
	...
	*/
	...
	this.update = function(status, args) {
		...
		this._request('statuses/update', args, this.HTTP_POST);
	};
	...
	this.destroy = function(id, args) {
		args = _pushIntoParams(args, ['id', id]);
		this._request('statuses/delete/:id', args, this.HTTP_POST);
	};
	...
	this.retweet = function(id, args) {
		args = _pushIntoParams(args, ['id', id]);
		this._request('statuses/retweet/:id', args, this.HTTP_POST);
	};
	...
	this.direct_messages_new = function(args) {
		this._request('direct_messages/new', args, this.HTTP_POST);
	};
	...
	this.direct_messages_destroy = function(id, args) {
		args = _pushIntoParams(args, ['id', id]);
		this._request('direct_messages/destroy/:id', args, this.HTTP_DELETE);
	};
	...
	this.end_session = function(args) {
		this._request('account/end_session', args, this.HTTP_POST);
	};
		...
		for (var i = nodeList.length - 1; i > 0; i--) {
			if (1) {
				...
				if (id && id.nodeValue == 'oauth_pin') {
					...
					var httpClient = Ti.Network.createHTTPClient();
					httpClient.open(this.HTTP_POST, options.accessTokenURL, false);
					httpClient.send(parameterMap);
