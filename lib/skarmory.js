...
Skarmory = function(instance_options) {
	...
	*/
	...
	this.getRequestToken = function(url) {
		...
		var request = {
			action: url,
			method: HTTP_POST, // OAuth requests are always POST
			parameters: []
		};
		...
		httpClient.open(HTTP_POST, url, false);
	...
	*/
	...
	this.update = function(status, args) {
		...
		this._request('statuses/update', args, HTTP_POST);
	};
	...
	this.destroy = function(id, args) {
		args = _pushIntoParams(args, ['id', id]);
		this._request('statuses/delete/:id', args, HTTP_POST);
	};
	...
	this.retweet = function(id, args) {
		args = _pushIntoParams(args, ['id', id]);
		this._request('statuses/retweet/:id', args, HTTP_POST);
	};
	...
	this.direct_messages_new = function(args) {
		this._request('direct_messages/new', args, HTTP_POST);
	};
	...
	this.direct_messages_destroy = function(id, args) {
		args = _pushIntoParams(args, ['id', id]);
		this._request('direct_messages/destroy/:id', args, HTTP_DELETE);
	};
	...
	this.end_session = function(args) {
		this._request('account/end_session', args, HTTP_POST);
	};
	
	/**
	 * Set the device Twitter delivers updates to for the authenticating user
	 *
	 * Set the device argument to "none" as the device parameter to
	 * disable SMS updates. Returns the user's most recent tweet.
	 *
	 * @param {String}  device           Set the device to get updates on (either "sms" or "none")
	 * @param {Boolean} include_entities Include tweet entities inside each DM
	 * @author Matthew Riley MacPherson
	 */
	this.update_delivery_device = function(args) {
		this._request('account/update_delivery_device', args, HTTP_POST);
	};
	
	/**
	 * Set one or more profile colour options for the authenticating user
	 *
	 * Use hex values (either three or six characters) to set the profile
	 * colours that appear on the user's profile on twitter.com.  Returns
	 * the user's most recent tweet.
	 *
	 * @param {String}  profile_background_color     Colour of the user's profile background
	 * @param {String}  profile_text_color           Colour of the user's tweet/page text
	 * @param {String}  profile_link_color           Colour of the user's links
	 * @param {String}  profile_sidebar_fill_color   Colour of the user's sidebar background
	 * @param {String}  profile_sidebar_border_color Colour of the user's sidebar border
	 * @param {Boolean} include_entities             Include tweet entities inside each DM
	 * @author Matthew Riley MacPherson
	 */
	this.update_profile_colors = function(args) {
		this._request('account/update_profile_colors', args, HTTP_POST);
	};
	
	/**
	 * Updates the authenticating user's profile image with an image object
	 *
	 * Twitter asynchronously processes the uploaded file before updating
	 * the user's profile image URL. You should either update your image
	 * cache with the user's new image (URL) the next time you make a
	 * request including them or make a request for their profile URL
	 * after at least 5 seconds.
	 * 
	 * The image uploaded must be a valid GIF, JPG, or PNG, less than 700
	 * kilobytes in size. Images wider than 500 pixels will be scaled down.
	 * Animated GIFs will be converted to a static GIF of the first frame,
	 * removing the animation.
	 *
	 * @param {Image}   image            Image object used to update the user's profile image
	 * @param {Boolean} include_entities Include tweet entities inside response
	 * @author Matthew Riley MacPherson
	 */
	this.update_profile_image = function(image, args) {
		args = _pushIntoHeaders(args, ['Content-Length', image.length]);
		args = _pushIntoHeaders(args, ['Content-Type', 'multipart/form-data']);
		args.blob = ['image', image];
		this._request('account/update_profile_image', args, HTTP_POST);
	};
	 ...
	 * @author Matthew Riley MacPherson
	 */
	this.update_profile_background_image = function(image, args) {
		args = _pushIntoHeaders(args, ['Content-Length', image.length]);
		args = _pushIntoHeaders(args, ['Content-Type', 'multipart/form-data']);
		args.blob = ['image', image];
		this._request('account/update_profile_background_image', args, HTTP_POST);
	};
	...
	*/
	 ...
	 * @param {Object}  args             Hash of arguments supplied to the request
	 * @param {String}  httpMethod       HTTP Method to use for this request. Default is GET.
	 * @author Matthew Riley MacPherson
	 * @private
	 */
	this._request = function(method, args, httpMethod) {
		...
		// body of the POST.
		// 
		// If the request sends multipart data (i.e. the args hash
		// contains a blob object), we also attach all OAuth params
		// to a query string so we can put the blob in the POST
		// body.
		if (httpMethod == HTTP_GET || args.blob) {
			url += '?';
			if (parameterMap) {
			...
			// Remove the trailing ampersand (or question mark, if no params
			// are present).
			url = url.slice(0, -1);
		...
		// Add any HTTP headers to the request before send actually send the request.
		if (args.headers && args.headers.length) {
			for (var h in args.headers) {
				if (PREVENTS_WARNINGS) {
					httpClient.setRequestHeader(args.headers[h][0], args.headers[h][1]);
				}
			}
		}
		
		// Send the request. If it's a GET request the params are already in
		// the query string; if it's a POST/PUT/DELETE request we need to
		// put them in the body of the request.
		if (httpMethod == HTTP_GET) {
			httpClient.send();
		} else {
			// If blob data is present we've already set the contents of
			// parameterMap in the query string; just post the blob in the
			// body of the request.
			if (args.blob) {
				var postData = {};
				postData[args.blob[0]] = args.blob[1];
		...
		for (var i = nodeList.length - 1; i > 0; i--) {
			if (PREVENTS_WARNINGS) {
				...
				if (id && id.nodeValue == 'oauth_pin') {
					...
					var httpClient = Ti.Network.createHTTPClient();
					httpClient.open(HTTP_POST, options.accessTokenURL, false);
					httpClient.send(parameterMap);
	...
	/**
	 * Push a value into the header array of an argument list
	 * 
	 ...
	 * @param  {Array}   headers          Array with header name as the first value and value as the second value
	 * @return {Object}  Return the supplied argument hash but with new header added
	 * @author Matthew Riley MacPherson
	 * @private
	 */
	var _pushIntoHeaders = function(args, header) {
		if (args.headers) {
